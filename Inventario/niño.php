...
// Realizar el INNER JOIN entre producto y tallas
$query = "SELECT p.*, t.talla 
          FROM producto p 
          INNER JOIN tallas t ON p.id_producto = t.fk_id_producto 
          WHERE p.categorias = 'niño' AND p.estado = 'disponible'";
...
<head>
    ...
    <title>Bling Compra - Sección Niño</title>
...
<div class="video-container">
    <video src="../imgs/Niño.mp4" autoplay muted loop></video>
        ...
        <div id="productos" class="col-md-8">
            <div class="row">
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    $productos[$row['id_producto']]['info'] = $row;
                    $productos[$row['id_producto']]['tallas'][] = $row['talla'];
                }
                ...
                foreach ($productos as $producto) {
                    ...
                    echo "<img src='" . htmlspecialchars($info['imagen']) . "' alt='Imagen'>";
                    echo '<div class="card-body">';
                    echo '<h5 class="card-title">' . htmlspecialchars($info['nombre']) . '</h5>';
                    echo '<p class="card-text">Precio: $' . number_format($info['precio_unitario'], 2, ',', '.') . '</p>';
                    echo '<form method="POST" action="../pruebaCarrito.php" class="add-to-cart-form">';
                    echo '<input type="hidden" name="idProducto" value="' . htmlspecialchars($info['id_producto']) . '">';
                    echo '<input type="hidden" name="idUsuario" value="' . htmlspecialchars($idUsuario) . '">';
                    echo '<div class="mb-3">';
                    echo '<label for="cantidad' . $info['id_producto'] . '" class="form-label">Cantidad (Máximo 20):</label>';
                    echo '<input type="number" name="cantidad" id="cantidad' . $info['id_producto'] . '" class="form-control" value="1" min="1" max="20" required>';
                    echo '</div>';
                    
                    echo '<div class="mb-3">';
                    echo '<label for="talla' . $info['id_producto'] . '" class="form-label">Talla:</label>';
                    echo '<select name="talla" id="talla' . $info['id_producto'] . '" class="form-select" required>';
